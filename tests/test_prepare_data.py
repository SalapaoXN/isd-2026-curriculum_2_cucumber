...
class PrepareDataTests(unittest.TestCase):
    ...
    def test_partial_corpus_skips_missing_programs_and_unknown_directories(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("prepare_data._run_extract") as run_extract, patch(
                "prepare_data._run_merge"
            ) as run_merge:
    ...
    def test_missing_scope_pages_are_omitted_cleanly(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            _touch_pages(root / "outputs" / "ocr" / "it", range(39, 46), "it")
    ...
    def test_empty_or_unknown_only_corpus_fails_clearly(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "outputs" / "ocr" / "it").mkdir(parents=True)
            (root / "outputs" / "ocr" / "other").mkdir()
    ...
    def test_only_existing_stage_commands_are_orchestrated(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            _touch_pages(root / "outputs" / "ocr" / "ait", range(1, 2), "ait")
            ...
            for call in run_extract.call_args_list:
                ...
                self.assertNotIn("evaluate.py", repr(call))
