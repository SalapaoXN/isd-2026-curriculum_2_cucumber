...
from src.pipeline.tools.evaluation import evaluate as evaluator
from src.pipeline.tools.evaluation.evaluate import (
    calculate_cer,
    calculate_field_wer,
    calculate_wer,
    ...
    def test_batch_pairs_coexist_and_second_invocation_overwrites_reports(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(evaluator, "REPORT_DIR", report_dir), patch.object(
                ...
                "argv",
                [
                    "src.pipeline.tools.evaluation.evaluate.py",
                    "--pair",
                    str(first_pred),
                    str(first_gt),
                    "--pair",
            ...
            with patch.object(evaluator, "REPORT_DIR", report_dir), patch.object(
                sys,
                "argv",
                ["src.pipeline.tools.evaluation.evaluate.py", str(first_pred), "--gt", str(first_gt)],
            ), redirect_stdout(io.StringIO()):
                evaluator.main()
