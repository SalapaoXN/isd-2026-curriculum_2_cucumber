...
from src.page_metadata import resolve_document_page
from src.rule_extractor import RulePage
from src.run_pipeline import _document_page_for_ocr, run_ocr
...
class OcrCliTests(unittest.TestCase):
    ...
    def test_supported_prefixes_map_to_inputs_and_outputs(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for prefix in ocr.SUPPORTED_PREFIXES:
                (root / "inputs" / prefix).mkdir(parents=True)
    ...
    def test_page_range_is_forwarded_and_no_gpu_is_preserved(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "inputs" / "it").mkdir(parents=True)
            with patch.object(ocr, "BASE_DIR", root), patch.object(
                ocr, "run_ocr"
            ) as run_mock:
                with patch.object(
    ...
    def test_ocr_writes_only_ocr_boundary_and_processes_all_pages_by_default(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("src.run_pipeline.OCREngine", FakeOCREngine), patch(
                "src.run_pipeline.Image.open", return_value=FakeImage()
            ):
