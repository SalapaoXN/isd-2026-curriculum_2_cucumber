"""FINAL-2: public-path E2E run of the Core Evaluation Set.

Loads tests/rag/fixtures/final_core_eval_v1.json and executes every
case through rag.qa.ask() with counting stubs. No production fixes
happen here; failures become FINAL-3 repair tasks only.
"""

import json
import re
import unittest
from pathlib import Path

from rag.qa import ask

FIXTURE = Path(__file__).resolve().parent / "fixtures" / "final_core_eval_v1.json"
DB_PATH = Path(__file__).resolve().parents[2] / "cucumber_outputs" / "runtime" / "curriculum.db"


def _counting_stub(calls, name, payload=None):
    def _stub(prompt: str) -> str:
        calls.append(prompt)
        if payload is None:
            raise AssertionError(f"{name} model must not be called")
        return payload() if callable(payload) else payload
    return _stub


def _intent_payload(spec):
    return json.dumps(
        {
            "intent": spec["intent"],
            "proposed_program": spec.get("proposed_program"),
            "proposed_plans": spec.get("proposed_plans", []),
            "proposed_years": spec.get("proposed_years", []),
            "proposed_semesters": spec.get("proposed_semesters", []),
            "course_codes": spec.get("course_codes", []),
            "topic": spec.get("topic"),
            "requested_facts": spec["requested_facts"],
            "judgement_dimension": spec.get("judgement_dimension"),
            "unresolved": [],
        },
        ensure_ascii=False,
    )


def _advisory_stub(prompt: str) -> str:
    match = re.search(r"GROUNDED_OPTIONS:\n(.*)$", prompt, re.S)
    options = json.loads(match.group(1))
    codes = [
        option.get("course_code")
        for option in options
        if isinstance(option.get("course_code"), str)
    ][:2]
    return "จากหลักฐานที่มี วิชา " + " และวิชา ".join(codes) + " น่าสนใจ"


def _claim_text(claim) -> str:
    return json.dumps(
        {"value": claim.value, "evidence": str(claim.evidence)[:500]},
        ensure_ascii=False,
        default=str,
    )


class FinalCoreEvalTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        doc = json.loads(FIXTURE.read_text(encoding="utf-8"))
        cls.cases = doc["cases"]

    def _run_case(self, case):
        intent_calls: list[str] = []
        structured_calls: list[str] = []
        answer_calls: list[str] = []
        stub = case.get("harness_stub") or {}
        intent_budget = case["budget"]["intent"]
        if stub.get("intent"):
            intent_stub = _counting_stub(
                intent_calls, "intent", _intent_payload(stub)
            )
        elif intent_budget == 0:
            intent_stub = _counting_stub(intent_calls, "intent")
        else:
            self.fail(f"{case['id']}: budget allows intent calls but no stub")
        if case["budget"]["structured"] == 0:
            structured_stub = _counting_stub(structured_calls, "structured")
        else:
            self.fail(f"{case['id']}: core set never budgets structured calls")
        if stub.get("advisory"):
            answer_stub = _counting_stub(answer_calls, "answer", _advisory_stub)
        else:
            # Budget-0 answer paths are structurally call-free (rendering
            # only calls a provided callable); None keeps them deterministic.
            self.assertEqual(case["budget"]["answer"], 0, f"{case['id']}: answer budget")
            answer_stub = None

        context = None
        response = None
        for turn in case["turns"]:
            kwargs = {
                "structured_model_callable": structured_stub,
                "answer_model_callable": answer_stub,
                "intent_model_callable": intent_stub,
            }
            if turn.get("context") == "previous":
                self.assertIsNotNone(context, f"{case['id']}: no context to reuse")
                kwargs["conversation_context"] = context
            response = ask(DB_PATH, turn["question"], **kwargs)
            context = response.get("next_context")
        result = response["result"]
        if isinstance(result, dict):
            status = result.get("status") or result.get("action")
        else:
            status = result.status
        self.assertEqual(status, case["expected_status"], f"{case['id']}: status")
        if isinstance(result, dict):
            self.assertEqual(case["claim_checks"], [], f"{case['id']}: blocked must be claimless")
            return
        for check in case["claim_checks"]:
            matches = [
                claim
                for claim in result.claims
                if claim.operation == check["operation"] and claim.status == check["status"]
            ]
            self.assertTrue(matches, f"{case['id']}: no claim {check}")
            if "value" in check:
                self.assertTrue(
                    any(claim.value == check["value"] for claim in matches),
                    f"{case['id']}: no claim value {check['value']}",
                )
            for text in check.get("contains", ()): 
                self.assertTrue(
                    any(text in _claim_text(claim) or text in result.final_answer for claim in matches),
                    f"{case['id']}: {text!r} not in claim/final",
                )
            if "scope_credit" in check:
                self.assertTrue(
                    any(
                        getattr(claim.effective_scope, "credit_units", None) == check["scope_credit"]
                        for claim in matches
                    ),
                    f"{case['id']}: scope credit mismatch",
                )
        for text in case["final_contains"]:
            self.assertIn(text, result.final_answer, f"{case['id']}: final missing {text!r}")
        if case["provenance_required"]:
            self.assertTrue(result.provenance, f"{case['id']}: provenance missing")
        self.assertEqual(len(intent_calls), intent_budget, f"{case['id']}: intent budget")
        self.assertEqual(len(structured_calls), 0, f"{case['id']}: structured budget")
        self.assertEqual(len(answer_calls), case["budget"]["answer"], f"{case['id']}: answer budget")

    def test_core_cases(self):
        failures = []
        for case in self.cases:
            with self.subTest(case_id=case["id"]):
                try:
                    self._run_case(case)
                except Exception as error:  # noqa: BLE001 - collected for the FINAL-2 report
                    failures.append((case["id"], str(error)[:200]))
                    raise
        self.assertEqual(failures, [])
        self.assertGreaterEqual(len(self.cases), 25)


if __name__ == "__main__":
    unittest.main()
