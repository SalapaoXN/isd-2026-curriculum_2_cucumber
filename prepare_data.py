...
This stage deliberately starts at ``outputs/ocr``.  It does not invoke OCR,
...
from merge_consecutive import merge_consecutive_files, parse_page_range
...
def _run_extract(
    ...
    if scope.plan is not None:
        command.extend(["--plan", scope.plan])
    if scope.pages is not None:
        command.extend(["--pages", scope.pages])
...
def prepare_data(
    ...
    ocr_root: str | Path = "outputs/ocr",
    extracted_root: str | Path = "outputs/extracted",
    consolidated_root: str | Path = "outputs/consolidated",
) -> dict:
